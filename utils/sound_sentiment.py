...
class SoundSentimentExtractor:
    ...
    def extract_sentiment(self, audio_path, smoothing_window):
        ...
        predictions_np = smooth_audio_sentiment(predictions_np, smoothing_window)
        print(predictions_np)
        return predictions_np, power_features


def smooth_audio_sentiment(sentiment_features, smoothing_window=0):

    # Step 1.1: Perform average downsampling to reduce the frequency of the optical signal
    sentiment_features = sentiment_features.reshape((-1, len(sentiment_features), 2))
