...
class TextualSentimentTranslator:
    ...
    def __init__(self, class_dictionary_path=join(config['basedir'], "models/imagenet.txt"), k_neighbors=5, temperature=1):
    ...
    def translate_sentiment(self, audio_sentiment, noise=0.1):
        ...
        song_words=[]
        for cur_index in idx:
          song_word = self.words[cur_index]
          song_words.append(song_word) 

        new_song_words = []
        for x in song_words:
          song_words = x.split(', ')
          # several word/phrases with the same v/a will be selected
          # choose the first one
          new_song_words.append(song_words[0])
          
        word_ids=[]
        for cur_index in idx:
          word_id = self.class_ids[cur_index] 
        ...
        for i in range(len(word_ids)):
            cur_class_vector = one_hot_from_int(word_ids, batch_size=len(word_ids))
            # If k = 1
            if self.database.kneighbors == 1:
        ...
        return class_vectors, noise_vectors, new_song_words
...
def create_affective_dictionary(lexicon_path=join(config['basedir'], "models/words.csv"),
    ...
    for id in imagenet:
        ...
        count = 0
        for word in desc:
          # 如果imagenet里的word有对应的v/a值，加到cur_vec表中
            if word in word_dict:
                cur_vec += word_dict[word]
                count += 1

        if count > 0:
            # 一个单引号内有多个词就把加起来的v/a取平均
